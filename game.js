...
function update() {
    ...
    if (gameOver) {
        ...
        createText(`Oyun Bitti`, board.width / 2, board.height / 2 - 25, 'center', 50);

        createText(`Skor: ${score}`, board.width / 2, board.height / 2 + 25, 'center');

        createText(`Tıkla ve yeniden başlat..`, (cols * blockSize) / 2, board.height - 50, 'center');

        return
    }

    // Write score
    createText(`Skor: ${score}`, 30, 40);
